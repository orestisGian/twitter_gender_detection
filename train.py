...
if len(sys.argv) != 2:
	print 'this script takes one argument: training dataset'
	sys.exit(0)
	
dataset = str(sys.argv[1])
searchObj = re.search( r'(.+)\.csv', dataset)
if not searchObj:
	print "Input must be a csv file of format: {name}.csv"
	sys.exit(0)
...
with open(dataset) as csvread:
	reader = csv.DictReader(csvread)
	
	for row in reader:

		profile_theme_color = row['theme_color']
		profile_theme_color = profile_theme_color.zfill(6)
		facepp_gender = row['faceplusplus_gender']
		genderize_gender = row['genderize_gender']
		genderize_probability = row['genderize_probability']
		genderize_count = row['genderize_count']
		



		j = int(profile_theme_color[0:2],16)
		k = int(profile_theme_color[2:4],16)
		l = int(profile_theme_color[4:6],16)
