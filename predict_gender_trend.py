...
def gender_prediction(trend,prob_threshold,total_items):
	...
	cricTweet = limit_handled(tweepy.Cursor(api.search, q=trend).items(total_items))
	...
	for i in xrange(len(predicted_hybrid_prob)):
		...
		if math.isnan(predicted_hybrid_prob[i][0]) or math.isnan(predicted_hybrid_prob[i][1]):
			predicted_hybrid_prob[i][0] = 0.5
			predicted_hybrid_prob[i][1] = 0.5
		
		if max(predicted_hybrid_prob[i]) < prob_threshold:
			unknown_count+=1
		elif predicted_hybrid_prob[i][0] >= predicted_hybrid_prob[i][1]:
			user['gender'] = 'female'
	...
	print female_count
	print male_count
	print 'MALE PERCENTAGE: %.2f%%' % ((male_count)*100.0 / (length))
	print 'FEMALE PERCENTAGE: %.2f%%' % ((female_count)*100.0 / (length))
	print 'UNKNOWN PERCENTAGE: %.2f%%' % ((unknown_count)*100.0 / (length))
